(function (window, u) {
	...
	fn.extend(Guadrant, {
		makeGuadrant: function () {
			...
			for(var i in posGuadrant) {
				...
				if(minArea < -180 && minArea >= -225) {
					minArea2 = minArea + 360;
					maxArea2 = 180;
					minArea = -180;
				}

				if(maxArea >= 180 && maxArea < 225) {
		...
		makerGuadrantList: function (startAngle, curAngle, origin) { 					// 构造路径象限队列
			...
			if(startAngle) {
			// 	var startGuadrant = Guadrant.getPosGuadrant(startAngle, origin);
				var startGuadrant = Guadrant.getPosGuadrant(startAngle, origin);
				var endGuadrant = Guadrant.getPosGuadrant(curAngle, origin);
				console.log(endGuadrant)
			// 	for(var i in startGuadrantLoad) {
			// 		if(~i.indexOf(startGuadrant)) {
			// 			filterGuadrantListString = startGuadrantLoad[i];
			// 			break;
			// 		}
			// 	}
			}else{
		...
		debugAngle: function (startAngle, endAngle, isClick) { 							// 解决鼠标从第二象限滑动到第三象限或者从第三象限滑动到第二象限的角度问题

			var	filterGuadrantListString = "", isReverse, 
				originalFilterGuadrantListString = "",
				data = Cache.data(dial, 'dataObj'),
				isEndHandler = data.isEndHandler;

			console.log(startAngle);
			if(!isClick) {
				originalFilterGuadrantListString = Guadrant.makerGuadrantList(null, endAngle, true); 		// 获取原始路径
				// filterGuadrantListString = Guadrant.makerGuadrantList(null, endAngle, null);			
				console.log(originalFilterGuadrantListString);
				isReverse = Guadrant.isReverse(originalFilterGuadrantListString, startAngle, endAngle); 	// 旋转方向
				...
				// filterGuadrantListString = Guadrant.makerGuadrantList(startAngle, endAngle, true);
				// isReverse = Guadrant.isReverse(filterGuadrantListString, startAngle, endAngle);
				...
				// 	}
				// }
				filterGuadrantListString = Guadrant.makerGuadrantList(null, startAngle);

				// console.log(filterGuadrantListString);
				// isReverse = Guadrant.isReverse(originalFilterGuadrantListString, startAngle, endAngle);
			...
			// 		endAngle -= 360;
			// 	}
			// 	if(isReverse && startAngle < 0 && endAngle > 0) {
			// 		startAngle += 360; 
			// 	}
			// 	if(!isReverse && startAngle > 0 && endAngle < 0) {
			// 		startAngle -= 360;
	...
	fn.extend(Handler, {
		startHandler: function (e) {
			...
			fn.extend(data, {
				...
				isEndHandler: false												// isEndHandler标记为false, 表示未进入endHandler事件
			});
		},
		moveHandler: function (e) {
			...
			moveAngle = Guadrant.debugAngle(startAngle, moveAngle); 				// 解决从第二象限到第三象限或者从第三象限到第二象限的角度问题						

			slideAngle = Handler.countSlideAngle.call(e, startAngle, moveAngle);	
		...
		countSlideAngle: function (startAngle, endAngle) {
			var data, angle, realAngle,
				slideAngle, moveClientPos;
			data = Cache.data(dial, 'dataObj');
			angle = data.angle; 

			if(data.clickClass && data.click) {	
				slideAngle = getCss.getRelativeAngle(endAngle, startAngle); 		// 计算该次的相对滑动角度
			}else {
				slideAngle = endAngle - startAngle; 							// 鼠标滑动的角度加上此前转盘旋转的角度
			}
		...
		countAngle: function (extraAngle, isClick) { 					// 方法countAngle(isClick)计算转盘旋转的角度
			var data = Cache.data(dial, 'dataObj'),
				...
				clickClass = data.clickClass,
				slideAngle = data.slideAngle,
				transition = options.transition, 
				curBlockChange = data.curBlockChange;

			// 加上相对滑动角度
			if(isClick) {
				// slideAngle = -Handler.countClickAngle();
				endAngle = Guadrant.debugAngle(endAngle, data.initAngle, true);
				slideAngle = Handler.countSlideAngle(endAngle, data.initAngle);
			}
		...
		countClickAngle: function () {
			var data = Cache.data(dial, 'dataObj'),
				initAngle = data.initAngle, angleObj,
				endAngle = data.endAngle,
				relativeAngle;

			angleObj = Guadrant.debugAngle(initAngle, endAngle, true);
			relativeAngle = getCss.getRelativeAngle(angleObj.startAngle, angleObj.endAngle);
	...
	var Dial = function (selector, opt) {
		...
		var dialElement = document.querySelector(selector),
			...
			initAngle = origalInitAngle - 90 || -90,					
