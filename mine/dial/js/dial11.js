(function (window, u) {
	...
	fn.extend(Guadrant, {
		...
		isReverse: function (listStr, startAngle, endAngle, isClick) {
			...
			if(listStr.length === 1) {  						
				...
				if(~reverseObj.isClick.isReverse.indexOf(guadrant)) { 					// 二三象限是顺时，一四象限是逆时
					reverse = true;
		...
		makerGuadrantList: function (curAngle, origin) { 					// 构造路径象限队列
		...
		isIndebugArea: function (angle) {
			var data = Cache.data(dial, 'dataObj'),
				area, areaNum, minArea, maxArea, isIndebugArea,
				debugPosGuadrant = data.debugPosGuadrant || posGuadrant;

			for(var i in debugPosGuadrant) {
				area = i.split(",");
				if(area[1]) {
					areaNum = area[1].split(" ");
					minArea = +areaNum[0];
					maxArea = +areaNum[1];
					if(angle > minArea && angle <= maxArea) {
						data.isIndebugArea = isIndebugArea;
						return true;
					}
				}
			}
		},
		debugAngle: function (endAngle, startAngle, isClick) { 							// 解决鼠标从第二象限滑动到第三象限或者从第三象限滑动到第二象限的角度问题
			...
			filterGuadrantListString = Guadrant.makerGuadrantList(endAngle);

			originalFilterGuadrantListString = Guadrant.makerGuadrantList(endAngle, true);

			isReverse = Guadrant.isReverse(originalFilterGuadrantListString, startAngle, endAngle, isClick);

			console.log(filterGuadrantListString, originalFilterGuadrantListString);
			...
			if(!isClick) {
				...
				if(!isReverse) { 				// 顺时针
					if(~originalFilterGuadrantListString.indexOf("3")) {
						endAngle -= 360;
					}
				}else{
					if(~originalFilterGuadrantListString.indexOf("2") ) {
						endAngle += 360;
					}
				}
			}
		...
		debugClickAngle: function (endAngle, startAngle) { 				// 
			var	filterGuadrantListString = "", 
				originalFilterGuadrantListString = "",
				posGuadrant,
	...
	fn.extend(Handler, {
		...
		countClickAngle: function () {
			...
			angleObj = Guadrant.debugClickAngle(initAngle, endAngle, true);
