(function (window, u) {
	...
	fn.extend(Guadrant, {
		...
		isReverse: function (listStr, startAngle, endAngle, isClick) {
			...
			if(listStr.length === 1) {  						
				...
				if(~isReverseLoad.indexOf(listStr)) { 										// 逆时针轨迹匹配到参数listStr，则认为是逆时针转动
					reverse = true;
				}else if(~reverseLoad.indexOf(listStr)){		
					reverse = false;
				}
			}
		...
		debugAngle: function (startAngle, endAngle, trigger) { 							// 解决鼠标从第二象限滑动到第三象限或者从第三象限滑动到第二象限的角度问题
			...
			if(isReverse) {  																			// 如果是逆时针
				if(~originalFilterGuadrantListString.indexOf("2 3")) { 									// 鼠标滑动的出现先到第二象限再到第三象限
	...
	fn.extend(Handler, {
		...
		countAngle: function (startAngle, endAngle, extra, trigger, state) { 												// 方法countAngle(isClick)计算转盘旋转的角度
			var data = Cache.data(dial, 'dataObj'),
		...
		endHandler: function (e) {
			...
			if(isClick) {
				Handler.countAngle(data.endAngle, data.initAngle, 0, true);
			}else{
				Handler.countAngle(null, null, 0);
			}
		...
		isClick: function (e) {
			...
			fn.each(fn.makeArray(eventTargets), function (item, i, eventTargets) {
				if(fn.contains(item, target) || item === target) {									// 如果点中了块元素其中的一个
					if(!guadrantList.length) { 			// 开始,结束的坐标点相等并且鼠标滑动的路径为空数组，说明鼠标是没有滑动的
						isClick = true;	
						return false;
					}
				}
	...
	Dial.prototype = {
		...
		next: function () {
			var data = Cache.data(dial, 'dataObj');
			if(!data.destroy) {
				Handler.countAngle(null, null, data.eachRotateAngle); 							
			}
		},	
		prev: function () {
			var data = Cache.data(dial, 'dataObj');
		...
		playTo: function (index) {
			...
			clickBlock = data.clickBlock;
			angle = data.angle;
			startClientPos = getCss.getCenterPos(clickBlock[index - 1]);
			startAngle = getCss.getAngleAndLen(startClientPos, data.centerPos).angle;

			data.guadrantList = [];
			data.originalGuadrantList = [];
			Handler.countAngle(startAngle, data.initAngle, 0, true);
		}
	};
