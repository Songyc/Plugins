(function (window, u) {
	...
	fn.extend(Guadrant, {
		...
		isReverse: function (listStr, startAngle, endAngle, isClick) {
			var data = Cache.data(dial, 'dataObj'),
				reverse, guadrant,
				isReverseStr = fn.repeat("1 2 3 4 ", 10),
				reverseStr = fn.repeat("1 4 3 2 ", 10);
	...
	fn.extend(Handler, {
		...
		countRealAngle: function (e) { 												// 方法countRealAngle(e)计算转盘随鼠标滑动的角度
			var data = Cache.data(dial, 'dataObj'),
				...
				curClientPos, curAngle, relativeAngle, realAngle, angleObj,
			...
			angleObj = Guadrant.debugAngle(curAngle, startAngle); 					// 解决从第二象限到第三象限或者从第三象限到第二象限的角度问题						
			relativeAngle = getCss.getRelativeAngle(angleObj.endAngle, angleObj.startAngle); 			// 计算该次的相对滑动角度
