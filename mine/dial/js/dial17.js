(function (window, u) {
	...
	fn.extend(Handler, {
		...
		moveHandler: function (e) {
			...
			slideAngle = Handler.countSlideAngle.call(this, startAngle, moveAngle, false);
			Handler.rotato.call(this, slideAngle + data.angle, data.transition, null, false);						// 动画
			
			// if(data.onSlideMove) {
			// 	// Dial.isOnSlideMove = true;
		...
		countAngle: function (startAngle, endAngle, extraAngle, trigger, status) { 												// 方法countAngle(isClick)计算转盘旋转的角度
			...
			if(data.onSlideEnd && Dial.countOnSlideEnd < 1) {														// 执行回调函数
				Dial.countOnSlideEnd++;
