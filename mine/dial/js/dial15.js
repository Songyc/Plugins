(function (window, u) {
	...
	fn.extend(setCss, {
		...
		setClickBlockPos: function (prefix) {
			...
			fn.each(fn.makeArray(elems), function (el, index, elems) {
				...
				totalAngle = eachRotateAngle * index + 90 + initAngle; 				// 计算块元素旋转角度
	...
	var Guadrant = {},
		...
		isReverseLoad = fn.repeat(" 1 2 3 4 ", 10),
		reverseLoad = fn.repeat(" 1 4 3 2 ", 10);
	fn.extend(Guadrant, {
		makeGuadrant: function () {
			var data = Cache.data(dial, 'dataObj'),
			...
			for(var i in posGuadrant) {
				...
				minArea = +area[0];
				maxArea = +area[1];
				minArea += origalInitAngle;
				maxArea += origalInitAngle;
				minArea2 = "";
				maxArea2 = "";
				
				if(minArea < -180 && minArea >= -270) {
					minArea2 = minArea + 360;
					maxArea2 = 180;
					minArea = -180;
				}

				if(maxArea >= 180 && maxArea < 270) {
					maxArea2 = maxArea - 360;
					minArea2 = -180;
					maxArea = 180;
				}
				
				minArea = Guadrant.debugInitAngle(minArea);
				...
				debugPosGuadrant[minArea + " " + maxArea + (minArea2 ? "," + minArea2 + " ": "") + (maxArea2 ? maxArea2 : "") ] = posGuadrant[i];
			}
			data.debugPosGuadrant = debugPosGuadrant;
			return debugPosGuadrant;
		},
		getPosGuadrant: function (angle, origin) { 											// 分象限
			var data = Cache.data(dial, 'dataObj'),
		...
		isReverse: function (list, startAngle, endAngle) {
			var data = Cache.data(dial, 'dataObj'),
				reverse, guadrant;
			...
			if(list.length === 1) {  						
				...
				if(~"1 4".indexOf(guadrant)) { 					// 二三象限是顺时，一四象限是逆时
					reverse = true;
				}else if(~"2 3".indexOf(guadrant)){
					reverse = false;	
				}
			}
			else {
				if(~isReverseLoad.indexOf(list)) { 										// 逆时针轨迹匹配到参数list，则认为是逆时针转动
					reverse = true;
				}else if(~reverseLoad.indexOf(list)){		
					reverse = false;
				}
			}
		...
		filter: function (guadrantList) {
			var filterGuadrantListString = "";
			fn.each(guadrantList, function (item, i, guadrantList) { 		// 数组去重
		...
		makerGuadrantList: function (curAngle, origin, one) { 					// 构造路径象限队列
			var guadrant, data = Cache.data(dial, 'dataObj'),
				guadrantList = origin ? data.originalGuadrantList : data.guadrantList,
			...
			guadrant = Guadrant.getPosGuadrant(curAngle, origin); 			// 获取当前鼠标坐标的象限
			guadrantList.push(guadrant);
			filterGuadrantListString = Guadrant.filter(guadrantList);
			origin ? (data.originalGuadrantList = guadrantList) : (data.guadrantList = guadrantList);
		
			return filterGuadrantListString;
		},
		debugInitAngle: function (angle) {
			if(angle <= -180) {
				angle += 360;
			} 
		...
		trigger: function (startAngle, endAngle) {
			...
			filterGuadrantListString = Guadrant.makerGuadrantList(startAngle);
				
			if(~" 1 4".indexOf(filterGuadrantListString)) {
				isReverse = true;
			}else if(~" 2 3".indexOf(filterGuadrantListString)) {
				isReverse = false;
			}

			originalFilterGuadrantListString1 = Guadrant.makerGuadrantList(startAngle, true, true);
			originalFilterGuadrantListString2 = Guadrant.makerGuadrantList(endAngle, true, true);
			...
			index1 = load.indexOf(originalFilterGuadrantListString1);
			index2 = load.indexOf(originalFilterGuadrantListString2);

			if(index1 > index2) {
				index2 = load.substr(index1).indexOf(originalFilterGuadrantListString2) + index1;
			}

			originalFilterGuadrantListString = load.substring(index1, index2 + 2); 

			return { endAngle: endAngle, originalFilterGuadrantListString: originalFilterGuadrantListString, isReverse: isReverse};
		},

		debugAngle: function (startAngle, endAngle, trigger) { 							// 解决鼠标从第二象限滑动到第三象限或者从第三象限滑动到第二象限的角度问题
			...
			if(!trigger) {
				originalFilterGuadrantListString = Guadrant.makerGuadrantList(endAngle, true); 				// 获取原始路径
				isReverse = Guadrant.isReverse(originalFilterGuadrantListString, startAngle, endAngle); 	// 获取旋转方向
				
			}else {
				reverseInfo = Guadrant.trigger(startAngle, endAngle);
			...
			if(isReverse) {  																// 如果是逆时针
				if(~originalFilterGuadrantListString.indexOf("2 3")) { 		
					endAngle -= 360; 														// 结束的角度减小360
					console.log(originalFilterGuadrantListString, endAngle);				// 鼠标滑动的出现先到第二象限再到第三象限
				}						
			}else if(!isReverse) {
				if(~originalFilterGuadrantListString.indexOf("3 2")) {
					endAngle += 360;														// 结束的角度增加360
				}
			}
			return {endAngle: endAngle, startAngle: startAngle} ;
		}
	})
	...
	fn.extend(Handler, {
		startHandler: function (e) {
			...
			fn.extend(data, {
				...
				slideAngle: 0,												// isEndHandler标记为false, 表示未进入endHandler事件
				isClick: false
			});
		},
		moveHandler: function (e) {
			...
			moveClientPos = getCss.getClientPos(e);											// 当前鼠标位置
			moveAngle = getCss.getAngleAndLen(moveClientPos, data.centerPos).angle; 		// 当前鼠标与中心的角度。以x的正方向为基准, 顺时针为正数, 逆时针为负数				
			console.log(data.isClick)
			slideAngle = Handler.countSlideAngle(startAngle, moveAngle);
			Handler.rotato(slideAngle + data.angle, data.transition);						// 动画
			fn.extend(data, { 														
		...
		countSlideAngle: function (startAngle, endAngle, trigger) {
			var data, angle, realAngle, angleObj,
				slideAngle, moveClientPos;
			data = Cache.data(dial, "dataObj");
			angle = data.angle;
			angleObj = Guadrant.debugAngle(startAngle, endAngle, trigger); 		// 解决从第二象限到第三象限或者从第三象限到第二象限的角度问题						
			
			if(data.clickClass && data.click) {	
				slideAngle = getCss.getRelativeAngle(angleObj.endAngle, angleObj.startAngle); 	// 计算该次的相对滑动角度
			}else {
		...
		countAngle: function (startAngle, endAngle, trigger) { 												// 方法countAngle(isClick)计算转盘旋转的角度
			var data = Cache.data(dial, 'dataObj'),
			...
			if(data.isClick || (startAngle !== null && endAngle !== null)) {
				slideAngle = Handler.countSlideAngle(startAngle, endAngle, trigger);
			}

			if(oneStep && !data.isClick) {
				slideAngle = Handler.oneStep(slideAngle);
			}
			
			angle += slideAngle; 	

			Handler.rotato(angle, data.transition, {
				angle: angle,
				curIndex: getCss.getBlockIndex(angle)								// 清空路径象限队列
			});
			...
			if(data.onChange) {														// 执行回调函数
				data.onChange.call(data);
			}
		},
		endHandler: function (e) {
			...
			if(isClick) {
				Handler.countAngle(data.endAngle, data.initAngle, true);
			}else{
				Handler.countAngle(null, null);
			}
		...
		playTo: function (index) {
			...
			if(!data.destroy) {
				Handler.countAngle(startAngle, data.initAngle, true);
			}
		},
		autoPlay: function (time) {
			...
			timer = setInterval(function () {
				Handler.playTo(data.curIndex + 1);
			}, time || 1500);
			data.timer = timer;
		},
		stop: function () {
	...
	Dial.prototype = {
		...
		next: function () {
			Handler.playTo(this.getCurIndex() + 1);
		},	
		prev: function () {
			Handler.playTo(this.getCurIndex() - 1);
		},
		playTo: function (index) {
			Handler.playTo(index - 1);
		},
		destroy: function () {
			Handler.destroy();
			data.destroy = true;
