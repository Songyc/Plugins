(function (window, u) {
	...
	fn.extend(setCss, {
		...
		setClickBlockPos: function (prefix) {
			...
			fn.each(fn.makeArray(elems), function (el, index, elems) {
				...
				totalAngle = eachRotateAngle * index + initAngle; 				// 计算块元素旋转角度，加上90度是为了让第一个块元素的方向指向x轴的正方向
				x = radius * ( Math.sin( Matrix.angleToRadian(totalAngle) ));
				y =  -radius * (Math.cos( Matrix.angleToRadian(totalAngle) ));
				
	            x = setCss.debugRadius(x); 											// 可能获取到的字符串含有e-字样, 设置为0
	...
	fn.extend(getCss, {
		...
		getRelativeAngle: function (curAngle, startAngle, isClick) {
			var data = Cache.data(dial, 'dataObj'),
				...
				totolAngle = relativeNumber * eachRotateAngle; 			

			return totolAngle; 														// 返回转动的相对角度
		},
	...
	fn.extend(Guadrant, {
		makeGuadrant: function () {
			...
			for(var i in posGuadrant) {
				...
				minArea = +area[0];
				maxArea = +area[1];
				minArea = +area[0] + initAngle;
				maxArea = +area[1] + initAngle;
				minArea2 = "";
				maxArea2 = "";
				
				if(minArea < -180) {
					minArea = area[0];
					minArea2 = initAngle + 180;
					maxArea2 = 180;
				}
				...
				if(maxArea > 180) {
					maxArea = area[1];
					minArea2 = -180;
					maxArea2 = initAngle - 180;
				}

				debugPosGuadrant[minArea + " " + maxArea] = posGuadrant[i];

				if(minArea2 && maxArea2) {
					debugPosGuadrant[minArea2 + " " + maxArea2] = posGuadrant[i];
				}
			}
		...
		getPosGuadrant: function (angle) { 											// 分象限
			var data = Cache.data(dial, 'dataObj'),
				guadrant = data.debugPosGuadrant,
		...
		makerGuadrantList: function (curAngle, origin) { 								// 构造路径象限队列
			var guadrant, 
				data = Cache.data(dial, 'dataObj'),
				guadrantList = origin ? (data.originGuadrantList || []) : (data.guadrantList || []),
			...
			origin ? data.originGuadrantList = guadrantList : data.guadrantList = guadrantList;
		...
		debugAngle: function (curAngle, startAngle, isClick) { 							// 解决鼠标从第二象限滑动到第三象限或者从第三象限滑动到第二象限的角度问题
			...
			var	filterGuadrantListString = "", 
				originFilterGuadrantListString = "",
				isReverse, filterGuadrantListString,
				data = Cache.data(dial, 'dataObj');

			filterGuadrantListString = Guadrant.makerGuadrantList(curAngle);			// 
			originFilterGuadrantListString = Guadrant.makerGuadrantList(curAngle, true);

			isReverse = Guadrant.isReverse(filterGuadrantListString, startAngle, curAngle, isClick);
			console.log(isReverse);
			if(!isClick) {
				if(isReverse) {  																// 如果是逆时针
					if(~originFilterGuadrantListString.indexOf("2 3")) { 						// 鼠标滑动的出现先到第二象限再到第三象限
						curAngle -= 360; 							// 结束的角度减小360
					}						
				}else if(!isReverse) {
					if(~originFilterGuadrantListString.indexOf("3 2")) {
						curAngle += 360;							// 结束的角度增加360
					}
				}
			}else{
				if(!isReverse) {
					if(~originFilterGuadrantListString.indexOf("3")) {
						curAngle -= 360;
					}
				}
			}
	...
	fn.extend(Handler, {
		...
		countClickAngle: function () {
			...
			relativeAngle = getCss.getRelativeAngle(endAngle, initAngle - 90, true);
