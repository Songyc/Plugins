(function (window, undefined) {
	var imgData = {
		screenObj:{width:640,height:1008}, //设计稿大小
		// width: 图片的实际宽 ;height: 图片的实际高 ; left: 图片对于设计稿的左边距 ; top: 图片对于设计稿的上边距
		page:{
			page1:{
				p1t1:{width:525, height:285, left:70, top:25},
				p1t2:{width:640, height:300, left:0, top:718},
				p1t3:{width:497, height:217, left:84, top:789},
				p1t4:{width:334, height:655, left:150, top:350},
				p1t5:{width:55, height:56, left:137, top:300},
				p1t6:{width:376, height:136, left:170, top:280},
				p1t7:{width:52, height:70, left:310, top:77},
				p1t8:{width:75, height:42, left:170, top:200},
				p1t9:{width:19, height:20, left:350, top:73},
				p1t10:{width:26, height:28, left:366, top:43},
				p1t11:{width:53, height:36, left:115, top:200}
			},
			page2:{
				p2t1:{width:188, height:211, left:25, top:25},
				p2t2:{width:405, height:97, left:215, top:60},
				p2t3:{width:255, height:238, left:104, top:195},
				p2t4:{width:389, height:495, left:243, top:170},
				p2t5:{width:92, height:58, left:378, top:455},
				p2t6:{width:475, height:569, left:0, top:400},
				p2t7:{width:534, height:74, left:71, top:740},
				p2t8:{width:227, height:34, left:228, top:829}
			}
		}
	};
	...
	setImgPos = function (cla, pos) {
		if(!cla) return;
		var	width , height,
			imgTop, imgLeft, imgWidth, imgHeight;
			obj = document.querySelector('.' + cla);

		imgLeft = ((pos.left / imgData.screenObj.width) * 100).toFixed(2); 
		imgTop = ((pos.top / imgData.screenObj.height) * 100).toFixed(2); 
		imgWidth = ((pos.width / imgData.screenObj.width) * 100).toFixed(2); 
		imgHeight = ((pos.height / imgData.screenObj.height) * 100).toFixed(2); 
		
		if(obj && obj.nodeType === 1) {
			obj.style.width = imgWidth + "%";
			obj.style.height = imgHeight + "%";
			obj.style.left = imgLeft + "%";
			obj.style.top = imgTop + "%";
			// obj.style.position = 'absolute';
		}
	},
	setAllImgPos = function (obj) {
		if(!obj) return;
		for(var key in obj) {
			setImgPos(key, obj[key]);		
		}
	},
	clearAllImgPos = function (page) {
		var obj = page.querySelectorAll('img');
	...
	var ImgPos = function (options) {
		var that = this,
			defaultOptions = {
				getAllImgPos: true
			};	
		options = this.extend(defaultOptions, options);
		if(options.getAllImgPos) {
			this.setAllImgPos(imgData.page);
		}else{
			setPageImgPos(1);
		}
		this.setImgPosStyleTag();
	}

	ImgPos.prototype = {
		constructor: ImgPos,
		setImgPosStyleTag: function () {
		...
		setAllImgPos: function (obj) {
			for(var key in obj) {
				setAllImgPos(obj[key]);	
			}
		},
		setPageImgPos: function (pageNum) {
			var pageObj = imgData.page["page" + pageNum];
			setImgPos(pageObj);
		},
		cleanImgPos: function (page) {
			clearAllImgPos(page);
		}
	}
	window.setImgPos = ImgPos;
})(window);
...
(function (window, undefined) {
	...
	var setHeight =  function () {
		doc.body.style.width = docW + "px";
		doc.body.style.height = docH + "px";
	}
...
(function (window, undefined) {
	var doc = document;
	//安卓手机的屏宽大小
	var setPhoneScale = function setPhoneScale() {
		...
		//    	"}else{"+
		//    		"document.write('<meta name="+'"viewport"'+" content=" + '"width=640, minimum-scale='+ phoneScale +', target-densitydpi=device-dpi "' + ">')"+
		//    	"}"+	
		// "}else{"+
		...
		if(/Android (\d+\.\d+)/.test(navigator.userAgent)){
			if(version > 2.3) {
				meta.setAttribute('content', 'width=640, minimum-scale=' + phoneScale +', maximum-scale=' + phoneScale + ', target-densitydpi=device-dpi');
			}else{
				meta.setAttribute('content', 'width=640, minimum-scale=' + phoneScale + ', target-densitydpi=device-dpi');
			}
		}else{
			meta.setAttribute('content', 'width=640, user-scalable=no, target-densitydpi=device-dpi,minimal-ui');
		}
		head.appendChild(meta);
	}
	window.setPhoneScale = setPhoneScale;
})(window);
...
(function (window, undefined) {
	...
	var doc = document,
		global = {}, arr = [],
		imgCount = 0, paths, isInit = true, keys = Object.keys,
		defaultOptions = {
			loadInfo: "Loading..." //加载信息。缺省值为Loading...
		},
		docW = (doc.documentElement || doc.body).clientWidth,
		docH = (doc.documentElement || doc.body).clientHeight;

	// public class
	var Preload = function (selector, options, callback) {
		var that = this,
			_thisOptions;
		if(!selector) return;
		...
		// 初始化
		that.init(selector, options);
		// 检测加载
		that.imgLoading(global.paths, callback);
	}
	// public function
	Preload.prototype = {
		constructor: Preload,
		...
		init: function (selector, options) {
			...
			//只有一个参数时
			else if(options === undefined && Object.prototype.toString.call(selector) === "[object Object]"){
				//创建节点
			...
			if(!options.paths) {
				...
				global = this.extend(defaultOptions, global);			
			}else{
				options = this.extend(defaultOptions, options);	
				global.paths = options.paths;
			}
			...
			if(isInit) {
				...
				data.className = "laoding-data";
				info.className = "laoding-info";
		...
		setStyle: function (obj, styleOptions) {
			var key;
			for(key in styleOptions) {
				obj.style.cssText += key + ":" + styleOptions[key] + ";";	
			}
		},
		imgLoading: function (paths, callback) {
			if(global.toString.call(paths) === "[object Object]") {
				// 图片资源预取
				var key,
					len = keys(paths).length,
					that = this;	
				global.length = len;
				global.info.innerHTML = global.loadInfo || global.options.loadInfo;
				for(key in paths) {
					var keyImg = key + "Img";
					paths[keyImg] = new Image();
					paths[keyImg].src = paths[key];
					paths[keyImg].onload = function () {
						imgCount++;
						global.data.innerHTML = Math.ceil((imgCount / len) * 100) + "%";
						if(imgCount === len) {
							that.imgLoaded();
							callback();
						}
		...
		imgLoaded: function () {
			Preload.imgLoaded = true;
			global.target.style.display = 'none';
		}
	}
	window.setPreload = Preload;	
})(window);
...
(function (window, undefined) {
	...
	Music.prototype = {
		...
		setMusicStyleTag: function () {
			...
			'}'+
			'.playMusic{'+
				'-webkit-animation: musicRotate 1.2s linear 0s infinite;'+
			'}';
			head.appendChild(style);
		},
		createEles: function () {
			...
			if(options.controls) {this.controls();}
			if(options.loop) {this.loop();}
			// 加载音乐背景
			options.imgStyle.background = "url(" + options.img + ") no-repeat";
			// 
			options.imgStyle['background-size'] = "100%";
			// 加载样式
			this.setStyle(img, options.imgStyle);
		},
		...
		setStyle: function (obj, style) {
			...
			for(var key in style) {
				obj.style.cssText += key + ":" + style[key]; 	
			}
		},
		initMusic: function () {
			if(!isInit) {
				this.createEles();
				this.setMusicStyleTag();
		...
		play: function () {
			this.addClass(img, 'playMusic');
			audio.play();
		},
		pause: function () {
			this.removeClass(img, 'playMusic');
			audio.pause();
		},
		autoPlay: function () {
			var options = global.options;
			audio.setAttribute("autoPlay", options.autoPlay);
			this.play();
			this.addClass(img, 'playMusic');
		},
		...
		loop: function () {
			var options = global.options;
			audio.setAttribute("loop", options.loop);
		},
		removeLoop: function () {
		...
		controls: function () {
			var options = global.options;
			if(!audio.controls) {
				audio.setAttribute('controls', options.controls);
			}
		},
...
(function (window, undefined){
	...
	var IScroll = function (options) {
		if(doc.querySelector('.' + options.target + '>div')) {
			var wrapper=doc.querySelector('.' + options.target),
				swiperWrap = options.snap;
			if(wrapper) wrapper.style.height = docH + "px";
			if(swiperWrap) {
				for(var i = 0 , l = swiperWrap.length; i < l; i++) {
					swiperWrap[i].style.height = docH + "px";
				}
			}
		}
		new iScroll(options.target, options);
	}
	window.setIScroll = IScroll;
})(window);
...
(function (window, undefined) {
	var doc = document,
		...
		head = doc.querySelector('head');
	var weiXin = function (fn) {
		script.src = 'http://js.3conline.com/wap/common/wxApi.min.js';
...
(function (window, undefined) {
	...
	var receiver = {
		...
		setMusic: setMusic,
		setImgPos: setImgPos,
		setIScroll: setIScroll,
		setWeiXin: setWeiXin
	};
	// setHeight命令
	var SetHeightCommand = function (receiver) {
		this.receiver = receiver;
		this.name = "setHeight";
	}

	SetHeightCommand.prototype = {
		setHeight: function () {
			this.receiver.setHeight();
		}
	}
	// setPhoneScale命令
	var SetPhoneScaleCommand = function (receiver) {
		this.receiver = receiver;
		this.name = "setPhoneScale";
	}

	SetPhoneScaleCommand.prototype = {
		setPhoneScale: function () {
			this.receiver.setPhoneScale();
		}
	}
	//setPreload命令
	var SetPreloadCommand = function (receiver) {
		this.receiver = receiver;
		this.name = "setPreload";
	}

	SetPreloadCommand.prototype = {
		setPreload: function (params, callback) {
			new this.receiver.setPreload(params, callback);
		}
	}
	//setMusic命令
	var SetMusicCommand = function (receiver) {
		this.receiver = receiver;
		this.name = "setMusic";
	}

	SetMusicCommand.prototype = {
		setMusic: function (params) {
			new this.receiver.setMusic(params);
		}
	}

	//setImgPos命令
	var SetImgPosCommand = function (receiver) {
		this.receiver = receiver;
		this.name = "setImgPos";
	}

	SetImgPosCommand.prototype = {
		setImgPos: function () {
			new this.receiver.setImgPos();
		}
	}
	//setIScroll命令
	var SetIScrollCommand = function (receiver) {
		this.receiver = receiver;
		this.name = "setIScroll";
	}

	SetIScrollCommand.prototype = {
		setIScroll: function (params) {
			new this.receiver.setIScroll(params);
		}
	}
	//setWeiXin命令
	var SetWeiXinCommand = function (receiver) {
		this.receiver = receiver;
		this.name = "setWeiXin";
	}

	SetWeiXinCommand.prototype = {
		setWeiXin: function (fn) {
			this.receiver.setWeiXin(fn);
		}
	}

	// 命令请求者
	var SetCommand = function (command, params, callback) {
		var commandFn = command[command.name];
		if(params) {
			if(callback) {
				commandFn.call(command, params, callback);
			} else {
				commandFn.call(command, params);
			}
		} else {
			commandFn.call(command);	
		} 
	}
	// initMobilePage class
	var initMobilePage = function (options) {
		var defaultOptions = {
			setHeight: true,	
			setPhoneScale: false,
			setPreload: false,
		...
		var preloadCallback = function () {
			if(options.setMusic) {
				new SetCommand(new SetMusicCommand(receiver), options.setMusic);
			}
			// 执行SetImgPosCommand
			if(options.setImgPos) {
				new SetCommand(new SetImgPosCommand(receiver));
			}
			if(options.callback) options.callback(); 
		}
		// 执行SetHeightCommand
		if(options.setHeight) {
			new SetCommand(new SetHeightCommand(receiver));
		}
		// 执行SetPhoneScaleCommand
		if(options.setPhoneScale) {
			new SetCommand(new SetPhoneScaleCommand(receiver));
		}
		// 执行SetPreloadCommand
		if(options.setPreload) {
			new SetCommand(new SetPreloadCommand(receiver), options.setPreload, preloadCallback);
		}else{
			preloadCallback();
			if(options.callback) options.callback();
		}
		// 执行SetIScrollCommand
		if(options.setIScroll) {
			new SetCommand(new SetIScrollCommand(receiver), options.setIScroll);
		}
		// 执行SetWeiXinCommand
		if(options.setWeiXin) {
			new SetCommand(new SetWeiXinCommand(receiver), options.setWeiXin);
		}

	}
	initMobilePage.prototype = {
		extend: function (target, source) {
			if(!target) return;
			for(var key in source) {
				target[key] = source[key];
	...
	window.initMobilePage = initMobilePage;
})(window);
